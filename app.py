...
st.set_page_config(page_title="Tax Auditor 2025", layout="wide")
...
<style>
    ...
    .metric-box { border: 1px solid #ddd; padding: 15px; border-radius: 8px; background: white; text-align: center; }
</style>
...
with col_in:
    ...
    tp_b1 = t1.number_input("Box 1 (Fed)", 0.0)
    tp_b16 = t2.number_input("Box 16 (NJ)", 0.0)
    tp_fwh = t3.number_input("Box 2 (Fed Wh)", 0.0)
    tp_loc = t4.selectbox("Work State", ["NJ", "NY"], key="tploc")
    tp_swh = t5.number_input("Box 17 (St Wh)", 0.0)
    ...
    if status == "Married Filing Jointly":
        ...
        sp_fwh = s3.number_input("Box 2 (Fed Wh)", 0.0, key="sp2")
        sp_loc = s4.selectbox("Work State", ["NJ", "NY"], key="sploc")
        sp_swh = s5.number_input("Box 17 (St Wh)", 0.0, key="sp17")
    ...
    # BROKERAGE FIXED: min_value=None allows negative numbers (losses)
    brokerage = i3.number_input("Net Brokerage Gain/Loss", min_value=None, value=0.0, help="Accepts negative values for losses.")
...
fed_deduction = max(RULES["Fed"]["std"], (mrtg_int + min(RULES["Fed"]["salt_cap"], (tp_swh + sp_swh + prop_tax))))
fed_taxable = max(0, fed_agi - fed_deduction)
fed_tax_pre = calc_tax(fed_taxable, RULES["Fed"]["brackets"])

# AOTC Credit (Education)
...
fed_tax_final = max(0, fed_tax_pre - actual_aotc)
fed_refund = (tp_fwh + sp_fwh) - fed_tax_final

# STATE (NJ): Loss = 0 offset against salary
...
with col_res:
    st.markdown("<div class='section-head'>Audit Results</div>", unsafe_allow_html=True)
    m1, m2 = st.columns(2)
    ...
    audit_data = {
        "Audit Point": ["Fed AGI", "Fed Deduction", "Education Credit", "NJ Gross", "NJ Credit (Sched G)"],
        "Value": [f"${fed_agi:,.0f}", f"${fed_deduction:,.0f}", f"${actual_aotc:,.0f}", f"${nj_gross:,.0f}", f"${nj_credit:,.0f}"]
    }
    st.table(pd.DataFrame(audit_data))

    fig = px.pie(names=["Tax", "Take-Home"], values=[fed_tax_final+nj_tax_final, (tp_b1+sp_b1)-fed_tax_final-nj_tax_final], hole=0.5)
    st.plotly_chart(fig, use_container_width=True)

    csv = pd.DataFrame(audit_data).to_csv(index=False).encode('utf-8')
    st.download_button("📥 Save Audit to CSV", data=csv, file_name="2025_tax_audit.csv", mime="text/csv")
